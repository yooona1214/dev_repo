...
def mainChat(kakaorequest):
    ...
    # 사용자별로 agent 생성 또는 가져오기
    user_id = kakaorequest["userRequest"]["user"]["id"]
    loader_mem.create_or_load_user_agent(user_id)

    # 답변 생성 함수 실행
    response_queue = q.Queue()
    print('1',response_queue.queue)
    ...
    while (time.time() - start_time < 4):
        if not response_queue.empty():
            ...
            print('6',response_queue.queue)

            response = response_queue.get()
            run_flag= True
            print("responseeeeeeeeeeeeeeeeeeeee")
            print('2',response_queue.queue)

            break
        # 안정적인 구동을 위한 딜레이 타임 설정
        time.sleep(0.01)
    print('5',response_queue.queue)

    # 3.5초 내 답변이 생성되지 않을 경우
    if run_flag== False:     
        response = timeover()
        print('3',response_queue.queue)
    
    print('4',response_queue.queue)
