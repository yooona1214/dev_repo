...
def fetch_LcL_freight_rates(
    origin, destination, cargo_weight, cargo_type, weight, length, width, height, units
):
    ...
    if response.ok:
        try:
            ...
            relevant_shipments = [quote for quote in data]
            ...
            if relevant_shipments:
                ...
                for shipment in relevant_shipments:
                    shipment_dict = {
                        ...
                        "rate": (
                            shipment["generalCargo"]
                            if shipment["generalCargo"]
                            else shipment["hazardousCargo"]
                        ),
        ...
        print(
            f"Error fetching data. Status code: {response.status_code}. Response content: {response.text}"
        )


LcL_freight_quote = fetch_LcL_freight_rates(
    "china", "kenya", 100, "general", 100, 10, 10, 10, "inches"
)
...
def fetch_air_freight_rates(
    origin, destination, cargo_weight, cargo_type, length, width, height, units
):
...
air_freight_quote = fetch_air_freight_rates(
    "usa", "kenya", 100, "general", 10, 10, 10, "inches"
)
...
def fetch_RoRo_freight_rates(
    ...
    if response.ok:
        try:
            ...
            relevant_shipments = [shipment for shipment in data]
...
RoRo_freight_quote = fetch_RoRo_freight_rates(
    "japan", "kenya", "audi", "a5", "coupe", "petrol", 1400, "2wd"
)
...
def get_rag_response(query):
    ...
    documents = parser.load_data(
        "C:\\Users\\user\\Documents\\Jan 2024\\Projects\\RAGs\\Files\\VehicleImport.pdf"
    )
    ...
    response = query_engine.query(
        "What is the age restriction for car that can be imported into Kenya?"
    )
    return response
