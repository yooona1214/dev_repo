...
rag_error = create_vector_store_as_retriever(data= error_data, str1="LG_Robot_Error_Guide", 
                                       str2="This is error manual.")
...
pandas_tool = PythonAstREPLTool(
                ...
                func=python.run,
                description = f"""
                Useful for when you need to get causes about symptom in pandas dataframe 'df'. 
...
- Error expert: If the customer says an error has occurred, refer to the documentation and guide the appropriate action.
...
User: 화면에 Error가 발생했다고 떠요.
AI: [Error expert]에게 전달
...
manual_prompt.input_variables= ['input', 'intermediate_steps', 'agent_scratchpad', 'chat_history']
...
#- Cause expert: Using only the causes in the 'Cause' column that match the symptoms derived by the symptom expert, ask the customer questions about the causes of the symptoms one by one using the Twenty Questions game method. At this time, you should ask questions in the order of most frequent, and if the customer says that all the reasons are not correct, tell them that you will connect them to customer service.
error_prompt.input_variables= ['input', 'intermediate_steps', 'agent_scratchpad', 'chat_history']
error_prompt.template = '''

You are Error expert for KT service robot, You are one of the experts in this team of four people.
- Symptom expert: When a customer describes their symptoms, refer to the 'Sentences in which the customer expressed their symptoms' column in the csv file and say the one most similar symptom based on the 'Symptoms' column.
- Cause expert: Ask customers one by one the causes in the 'Cause' column that match the symptoms derived by the symptom expert in the Twenty Questions game. At this time, ask in order of the most frequent causes.
- Action expert: Once the cause is identified, appropriate action will be taken. If dispatch for repairs is necessary, tell the customer that you will accept dispatch.
- Manual expert: Respond appropriately when customers ask questions about specific usage of the robot or structural features of the robot hardware.
- Error expert: If the customer says an error has occurred, ask for the error code (including detailed code if necessary) and explain the cause of the error. They will guide you through what you can do and tell them that if the error continues to occur, they will report it to the customer service center for repair.
```
(Example)
User: 로봇 화면에 에러가 발생했다고 떠요.
AI: [Error expert] 화면에 보여지는 에러코드를 알려주실 수 있을까요?
User: 응, 4092야?
AI: [Error expert] 혹시 상세코드도 알려주실 수 있나요?
User: 상세 코드는 없어.
AI: [Error expert] 네, 감사합니다. 해당 에러는 HW에러가 원인입니다. 우선 직접 조치하여 로봇을 사용하실 수 있는 방법을 알려드리겠습니다. 그럼에도 불구하고 에러가 계속된다면 알려주시길 바랍니다.
AI: ...
...
pandas_agent = create_pandas_dataframe_agent(llm=llm_model,
                                      df=df_for_pandas,
                                      verbose=True,
                                      prefix='Please find the symptoms of client in the Symptom Category column and list the corresponding causes in order of frequency. Please answer all questions concisely in Korean.',
...
error_agent = create_openai_functions_agent_with_history(llm_model, tool_error, error_prompt)
...
while True:
    ...
    elif 'Error' in general_res :
        ask_error = '{chat_history}에서 고객의 error code에 대한 조치방안을 설명해주세요.'
        error_agent_executor = AgentExecutor(agent=error_agent, tools=tool_error, verbose=True)
        response= error_agent_executor.invoke({"input": ask_error, "chat_history": chat_history})
        chat_history.extend(
            [
                AIMessage(content=ask_error),
                AIMessage(content=response["output"]),
            ]
        )
