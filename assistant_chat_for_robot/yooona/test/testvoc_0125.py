...
def main():
    ...
    while(True):
        ...
        # print(Cause_list.iloc[1]["원인"])
        # print(Cause_list.iloc[0]["원인"])
        ...
        for index, row in Cause_list.iterrows():
            ...
            question = test_llm.invoke(str({row['원인']}) +'을 발생시키는 행동을 최근에 한 적 있는지 친절하게 한문장으로 물어봐주세요.') 
            print('\nAI:',question)
            ...
            result = test_llm.invoke('prompt:고객의 말이 긍정이거나 애매하면 yes, 부정이면 no를 반환해라. 고객의 말:'+user_answer)
            #print(result)   
            ...
            if user_answer == yes:
                if row['고객조치가능여부'] == True:
                    ans = test_llm.invoke(str(row['조치 방법'])+'을 통해 문제를 해결해보라고 권유 해주는 문장을 말해주세요.')
                    print('\nAI:',ans)
                ...
                if row['고객조치가능여부'] == False:
                    ans1 = test_llm.invoke(f"원인은 {str(row['원인'])} 이고  조치 방법은 {str(row['조치 방법'])} 입니다. 해당 원인을 해결하기 위해 다음과 같은 조치방법을 취하기 위해서 직원을 연결해드린다고 대답해주세요.")
