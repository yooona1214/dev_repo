...
'''
1. agent 없이 LLM모델에 tool기반으로 검색하게하기
2. csv 전처리 이후 원인 dataframe을 행 순서대로 하나씩 LLM.invoke할 수 있도록하는 구조
2-1. 원인 데이터를에 집어넣고 이 원인을 야기한 행동을 최근에 한 적 있는지 물어 보는 promlt LLM invoke하고, 
2-2. 그에 대한 대답이 긍정인지 부정인지 판별하는 prompt LLM invoke
2-2. 긍정이라면, 사용자 조치 여부에 따라 조치여부를 알려주거나, 출동 하도록함.
2-3. 부정이라면, 죄송합니다. 다른 원인을 알아볼게요. 라고 발화 후, 2-1 부터 반복
3. 2번의 시퀀스에 Multi prompt chain 적용
'''