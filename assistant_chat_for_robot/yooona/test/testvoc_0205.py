...
def create_agent_without_tool(llm, system_message: str):
    """Create an agent without tool."""
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                "You are a helpful AI assistant, collaborating with other assistants."
                " Use the provided tools to progress towards answering the question."
                " If you are unable to fully answer, then prefix your response with FINAL ANSWER and wait for human answer, {messages}."
                " Answer in korean"
                " \n{system_message}"
                "\nchat_history:{history}"
            ),
...
rag_agent = create_agent(llm, [rag], system_message="{messages} 기반으로, 어떤 이슈분류에 해당하는지 검색하고, 가장많이 검색된 {이슈분류}를 return 해주세요.")
rag_node = functools.partial(agent_node, agent=rag_agent, name="Researcher")


cause_classification_agent = create_agent(llm, [preprocess_csv_tool], system_message="{이슈분류}를 기반으로 Dataframe을 return해주세요.")
class_node = functools.partial(agent_node, agent=cause_classification_agent, name="Data Classificator")

consulting_agent = create_agent_without_tool(llm, system_message="너는 서비스 로봇 고장신고 처리 담당자 중 질문하는 역할이야. {result_df}의 {issue_counts.index}를 유발하는 행동을 혹시 최근에 한적 있는지 고객에게 친절하게 한문장으로 질문해주세요.")
consulting_node = functools.partial(agent_node, agent=consulting_agent, name="Consultant")

decision_agent = create_agent_without_tool(llm, system_message="너는 서비스 로봇 고장신고 처리 담당자 중 결정을 내리는 역할이야.{history}를 기반으로 고객을 응대해주세요. {message}를 듣고, {result_df}를 참고하여 그에 맞는 원인을 해결하기 위한 조치 방법을 알려주거나 서비스 기사를 출동시켜드리겠다라고 말하는 액션을 취해주세요.  ")
decision_node = functools.partial(agent_node, agent=consulting_agent, name="DecisionMaker")
...
workflow.add_node("Data Classificator", class_node) # 원인 정렬기
...
workflow.add_conditional_edges(
    "Researcher",
    router,
    {"continue": "Data Classificator", "call_tool": "call_tool", "end": END},
)
workflow.add_conditional_edges(
    "Data Classificator",
    router,
...
workflow.add_conditional_edges(
    "Consultant",
    router,
    {"continue": "DecisionMaker", "call_tool": "call_tool", "end": END},
)
...
workflow.add_conditional_edges(
    ...
    lambda x: x["sender"],
    {
        ...
        "Data Classificator": "Data Classificator",
