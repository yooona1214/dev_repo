...
members = ['Researcher', 'Cause Classificator', 'Consultant']
...
cause_classification_agent = create_agent(llm, [preprocess_csv_tool], system_message="{이슈분류}를 기반으로 Dataframe을 return하세요.")
class_node = functools.partial(agent_node, agent=cause_classification_agent, name="Cause Classificator")

consulting_agent = create_agent(llm, None, system_message="{last_message}를 기반으로 원인을 유발하는 행동을 혹시 최근에 한적 있는지 고객에게 친절하게 한문장으로 질문하세요.")
