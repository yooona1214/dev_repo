...
from kt_msgs.msg import BoxState, BoxStates, BoxStates, LEDState, Location, LEDEffect
...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_unload1')        
...
def tray_action_seq():
    ...
    seq.add_children([tray_open, hri_finished_check])
...
def go_to_next_state():
    ...
    no_task_check = compareBBvariableforcorrect(BB_variable1 = 'a',
                                         BB_variable2 = 'current_goal_count')
    
    next_task_check = compareBBvariableforincorrect(BB_variable1 = 'a',
                                         BB_variable2 = 'current_goal_count')
    
    #setBlackboard
    
    count_goal_index_num = Add_1('goal_index_num')
    seq1 = py_trees.composites.Sequence()
    seq2 = py_trees.composites.Sequence()
    ...
    go_to_idle1 = EnqueueNextService(
    ...
    tts_delivering_finished2= TTSPlay(tts_name='delivering_finished', play=True, sequence=True)
    ...
    seq1.add_children([no_task_check, tray_current_set, par1])
    seq2.add_children([next_task_check, count_goal_index_num, unset_route, route_to_BB, par_route_ready_check, tts_delivering_resume, task_finished_alarm_task_not_done, opened_tray_check_and_close(),go_to_moving_tree])
    ...
    seq3.add_children([tray_object_sensing_full_check, tts_delivering_finished2, task_fail_alarm, set_BB_fail, go_to_idle1])
    seq4.add_children([tray_object_sensing_empty_check, tts_delivering_finished, task_finished_alarm, go_to_idle2 ])
    return seq
...
def create_root():
    ...
    root_sel = py_trees.composites.Selector('tray_unload1')
    ...
    root.add_children([check_critical_error(), go_to_minor_error_tree(), item_return_check(), minor_error_alarm(), msgs_to_BB_parallel(),check_manual_drive(), check_manual_charge(), emergency_button_check(), battery_under_15(), time_delay_check_seq(), tray_stuck_seq_new(), main_seq ])

    main_seq.add_children([LED_charging_f, LED_charging_r, bb_start, get_tray_info, tray_action_start_tts, task_available_alarm, user_password_check_seq(), tray_action_seq(), go_to_next_state()])
    return root_sel
