...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_unload2')
...
def item_return_check():
    ...
    item_return_unset1 = SetBlackBoard(BB_variable='item_return_check', BB_value=False)


    seq.add_children([item_return_check1, send_back_alarm1, item_return_unset1])
...
def go_to_next_state():
    ...
    par_1_only_full = py_trees.composites.Parallel(
        name="par_1_only_full",
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    )

    par_2_only_full = py_trees.composites.Parallel(
        name="par_2_only_full",
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    )

    full_stuff_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'full_stuff_check',
        check=py_trees.common.ComparisonExpression(
    ...
    full_stuff_check2 = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'full_stuff_check2',
        check=py_trees.common.ComparisonExpression(
    ...
    empty_stuff_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'empty_stuff_check',
        check=py_trees.common.ComparisonExpression(
        variable="Tray_1_stuff_status",
        value=False,
        operator= operator.eq
    ...
    empty_stuff_check2 = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'empty_stuff_check2',
        check=py_trees.common.ComparisonExpression(
        variable="Tray_2_stuff_status",
        value=False,
        operator= operator.eq
    ...
    seq_full_stuff = py_trees.composites.Sequence()
    seq_1_only_full = py_trees.composites.Sequence()
    seq_2_only_full = py_trees.composites.Sequence()
    seq_empty_stuff = py_trees.composites.Sequence()

    par_full_stuff_check = py_trees.composites.Parallel(
        name='par_full_stuff_check',
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    )

    par_empty_stuff_check = py_trees.composites.Parallel(
        name='par_empty_stuff_check',
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    )

    par_half_stuff = py_trees.composites.Parallel(
        name='par_half_stuff',
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    ...
    tray1_full_check = py_trees.behaviours.WaitForBlackboardVariableValue(
            name= 'tray1_full_check',
            check=py_trees.common.ComparisonExpression(
            variable="Tray_1_stuff_status",
            value=True,
            operator= operator.eq
    ...
    tray2_full_check = py_trees.behaviours.WaitForBlackboardVariableValue(
            name= 'tray2_full_check',
            check=py_trees.common.ComparisonExpression(
            variable="Tray_2_stuff_status",
            value=True,
            operator= operator.eq
    ...
    set_next_current_tray_id1 = SetBlackBoard(BB_variable='2/current_tray_id', BB_value=[1])
    set_next_current_tray_id2 = SetBlackBoard(BB_variable='2/current_tray_id', BB_value=[2])
    ...
    seq.add_children([opened_tray_check_and_close(), go_to_next_state_passed_check, set_go_to_next_state, par_tray_all_closed, par])

    par_tray_all_closed.add_children([tray1_closed_check, tray2_closed_check])

    par.add_children([seq_full_stuff, par_half_stuff, seq_empty_stuff])
    
    seq_full_stuff.add_children([par_full_stuff_check, for_idle(alarm_type='task_fail', tray_id_id=[1, 2])])
    par_full_stuff_check.add_children([full_stuff_check, full_stuff_check2])

    par_half_stuff.add_children([seq_1_only_full, seq_2_only_full])
    seq_1_only_full.add_children([par_1_only_full, set_next_current_tray_id1, for_idle(alarm_type='task_fail', tray_id_id=[2])])
    par_1_only_full.add_children([tray1_full_check, tray2_empty_check])

    seq_2_only_full.add_children([par_2_only_full, set_next_current_tray_id2, for_idle(alarm_type='task_fail', tray_id_id=[1])])
    par_2_only_full.add_children([tray2_full_check, tray1_empty_check])

    seq_empty_stuff.add_children([par_empty_stuff_check, for_idle(alarm_type='task_finished', tray_id_id=[1,2])])
    par_empty_stuff_check.add_children([empty_stuff_check, empty_stuff_check2])

    return seq


def for_idle(alarm_type, tray_id_id):

    seq = py_trees.composites.Sequence()

    tts_delivering_finished= TTSPlay(tts_name='delivering_finished', play=True, sequence=True)


    task_alarm = PublishTopic(
                            name='task_alarm',
                            topic_name='ktmw_bt/task_status',
                            topic_type = TaskStatus, #TODO
                            msg_generate_fn=lambda: TaskStatus(
                                ...
                                current_goal_id='',
                                tray_id=tray_id_id,             
                                task_status = alarm_type,
                                action = alarm_type,
                                action_target = str(py_trees.blackboard.Blackboard.get('tray1')),
                                action_result = True
                                )   
                            )
    ...
    seq.add_children([tts_delivering_finished, task_alarm, go_to_idle_tree])
    return seq
...
def msgs_to_BB_parallel():
    ...
    def on_msg_fn_pw_req(msg):
        py_trees.blackboard.Blackboard.set('user_pw_type', msg.type) #user/admin 
        py_trees.blackboard.Blackboard.set('user_pw_tray_id', msg.tray_id) #string
        py_trees.blackboard.Blackboard.set('user_pw', msg.pw) #uint8

    user_pw_req_msgs_to_BB =  SubscribeTopic(
    ...
    par.add_children([error_set_to_BB, item_return_msgs_to_BB, button_status_to_BB, battery_state_to_BB, charging_state_to_BB, user_input_msgs_to_BB, 
                      user_pw_req_msgs_to_BB,emergency_button_info_to_BB,  tray_open_status_to_BB, remote_unlock_status_to_BB, box_exception_to_BB])
...
def user_password_check_seq():
    ...
    check_lock_option_auto = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_lock_option_auto',
                check=py_trees.common.ComparisonExpression(
                variable='goal_index_num_current_lock_option',
                value=1, #1: auto, 2: pw
                operator= operator.eq
            )
        )
    
    confirm_button_touched_check = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'confirm_button_touched_check',
                check=py_trees.common.ComparisonExpression(
                variable='hri_status',
                value= 'Arrived_check',
                operator= operator.eq
    ...
    check_lock_option_pw = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_lock_option_pw',
                check=py_trees.common.ComparisonExpression(
                variable='goal_index_num_current_lock_option',
                value=2, #1: auto, 2: pw
                operator= operator.eq
    ...
    unset_pw_req1 = SetBlackBoard(BB_variable='user_pw_type', BB_value='')
    unset_pw_req2 = SetBlackBoard(BB_variable='user_pw_type', BB_value='')
    ...
    pw_correct_check = compareBBvariableforcorrect(BB_variable1 = 'tray1_pw',
                                         BB_variable2 = 'user_pw')
    
    pw_incorrect_check = compareBBvariableforincorrect(BB_variable1 = 'tray1_pw',
                                         BB_variable2 = 'user_pw')
    ...
    failed_count_alarm = PublishTopic(
                            ...
                            msg_generate_fn=lambda: TaskStatus(
                                ...
                                current_goal_id='',
                                tray_id=[0],             
                                task_status = 'unload_tray_pw_check',
                                action = 'user_auth_'+str(3-py_trees.blackboard.Blackboard.get('pw_count')),
                                action_target = 'tray_'+str(py_trees.blackboard.Blackboard.get('tray1')),
                                action_result = False
                                )   
                            )
    ...
    unset_user_pw1 = SetBlackBoard(BB_variable='user_pw', BB_value='')
    unset_user_pw2 = SetBlackBoard(BB_variable='user_pw', BB_value='')



    
    pw_incorrect_check2 = compareBBvariableforincorrect(BB_variable1 = 'tray1_pw',
                                         BB_variable2 = 'user_pw')
    

    
    unset_pw_req1_1 = SetBlackBoard(BB_variable='user_pw_type', BB_value='')
    unset_user_pw1_1 = SetBlackBoard(BB_variable='user_pw', BB_value='')
    ...
    # led_start_on_pw_fail_f = LEDControl(
    #     color=LEDControl.COLOR_BLUE,
    #     position=LEDControl.POSITION_FRONT,
    #     x=1,
    #     y=1,
    ...
    # led_start_on_pw_fail_r = LEDControl(
    #     color=LEDControl.COLOR_BLUE,
    #     position=LEDControl.POSITION_REAR,
    #     x=1,
    #     y=1,
    ...
    led_start_on_pw_fail_f = PublishTopic(
                name='led_start_on_pw_fail_f',
                topic_name='led/set_led_effect_type',
                topic_type = LEDEffect,
                msg_generate_fn=lambda: LEDEffect(
                    cmd_id=0,
    ...
    led_start_on_pw_fail_r = PublishTopic(
                name='led_start_on_pw_fail_r',
                topic_name='led/set_led_effect_type',
                topic_type = LEDEffect,
                msg_generate_fn=lambda: LEDEffect(
                    cmd_id=0,
    ...
    par6 = py_trees.composites.Parallel(
        name="par6",
        policy=py_trees.common.ParallelPolicy.SuccessOnOne(
        )
    )
    ...
    check_over1 = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_over1',
                check=py_trees.common.ComparisonExpression(
                variable='pw_count',
                value=1,
    ...
    check_final = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_final',
                check=py_trees.common.ComparisonExpression(
                variable='pw_count',
                value=1,
    ...
    par_final_fail = py_trees.composites.Parallel(
        name="par_final_fail",
        policy=py_trees.common.ParallelPolicy.SuccessOnOne(
        )
    )

    seq_time = py_trees.composites.Sequence()
    seq_rm_request = py_trees.composites.Sequence()

    time_check = NoResponseCheck(time1='standby_duration')
    
    set_correct_pw = GetandSetBlackBoard(BB_variable='user_pw', BB_value='current_tray_pw')

    get_start_time = GetCurrentTime()

    set_flag_for_final_fail_time_T = SetBlackBoard('flag_for_final_fail_time', True)
    set_flag_for_final_fail_time_F = SetBlackBoard('flag_for_final_fail_time', False)

    def request_generate_fn_tray_pw():
        req = GetFile.Request()
        req.type = 'auth'
        req.version = '1.0.0'
        req.file = 'user_tray_1_pw.json'
        return req
    
    def response_fn_tray_pw(response):
        re = response.result
        val = response.data.string_value
        j_value = json.loads(val)
        pw = j_value['code_1']

        py_trees.blackboard.Blackboard.set('tray1_pw',pw)
        #print(response.result)

    tray_pw_msgs_to_BB = ServiceCall('data_manager/get_file_decrypt', GetFile, request_generate_fn_tray_pw,
                                         response_fn_tray_pw, name='tray_pw_msgs_to_BB')

    seq.add_children([pwcount_reset, par1])
    par1.add_children([seq1, seq2])
    seq1.add_children([check_lock_option_auto, confirm_button_touched_check])

    seq2.add_children([check_lock_option_pw, tray_pw_msgs_to_BB, par2, unset_user_pw1, unset_pw_req1 ])
    par2.add_children([seq3, seq4, seq5])
    seq3.add_children([check_over1, wait_for_pw_req, pw_incorrect_check, unset_user_pw2, unset_pw_req2, reduce_one_count, tts_password_fail1,led_start_on_pw_incorrect_f, led_start_on_pw_incorrect_r, failed_count_alarm])

    seq4.add_children([check_final, wait_for_pw_req2, pw_incorrect_check2, final_failed_alarm, led_start_on_pw_fail_f, led_start_on_pw_fail_r ,tts_password_lock, unset_pw_req1_1, unset_user_pw1_1, pw_correct_set2, service_locked_alarm, par_final_fail])
    par_final_fail.add_children([seq_time, seq_rm_request])
    seq_time.add_children([get_start_time, set_flag_for_final_fail_time_T, time_check, go_to_next_state()])
    seq_rm_request.add_children([admin_remote_auth, set_flag_for_final_fail_time_F, set_correct_pw])

    seq5.add_children([pw_correct_check,  auth_checkced_alarm, led_start_on_pw_done_f, led_start_on_pw_done_r])
...
def time_delay_check_seq():
    ...
    tray_action_seq_manual_nono = SetBlackBoard(BB_variable='meomchwi', BB_value=True)
    ...
    seq_no_res.add_children([no_response_check(time11='user_input_duration'), tts_user_input_delayed, user_input_delay_alarm, tray_action_seq_manual_nono, go_to_next_state()])
    return seq
...
def tray_action_seq_manual():
    ...
    delay_true_check1 = py_trees.behaviours.WaitForBlackboardVariableValue(
            name= 'delay_true_check1',
            check=py_trees.common.ComparisonExpression(
            variable="meomchwi",
            value=False,
            operator= operator.eq
    ...
    delay_true_check2 = py_trees.behaviours.WaitForBlackboardVariableValue(
            name= 'delay_true_check2',
            check=py_trees.common.ComparisonExpression(
            variable="meomchwi",
            value=False,
            operator= operator.eq
    ...
    tray1_open_seq.add_children([wait_hri_tray1_id_cmd, tray_not_ing_check(), tray_close2, tray2_closed_check, delay_true_check1, tray_open1, tray_id_unset1, tray1_closed_check_3])
  
    tray2_open_seq.add_children([wait_hri_tray2_id_cmd, tray_not_ing_check(), tray_close1, tray1_closed_check, delay_true_check2, tray_open2, tray_id_unset2, tray2_closed_check_3])

    #tray1_open_seq.add_children([wait_hri_tray1_id_cmd, tray2_closed_check, delay_true_check1, tray_open1, tray_id_unset1, tray1_closed_check_3])
  
    #tray2_open_seq.add_children([wait_hri_tray2_id_cmd, tray1_closed_check, delay_true_check2, tray_open2, tray_id_unset2, tray2_closed_check_3])
...
def create_root():
    ...
    par_stuff_check = py_trees.composites.Parallel(
        name="par_stuff_check",
        policy=py_trees.common.ParallelPolicy.SuccessOnOne(
        )
    )  


    tray_1_full_seq = py_trees.composites.Sequence()
    tray_1_empty_seq = py_trees.composites.Sequence()

    tray2_empty_seq = py_trees.composites.Sequence()
    tray2_full_seq = py_trees.composites.Sequence()

    tray_2_check_par = py_trees.composites.Parallel(
        name="tray_2_check_par",
        policy=py_trees.common.ParallelPolicy.SuccessOnOne(
        )
    )

    tray_open_1 = BoxOpen(position=1, x=1, y= 1) # first, tray num1 open automatically
    tray_open_1_2 = BoxOpen(position=1, x=1, y= 1) # first, tray num1 open automatically

    tray_open_2 = BoxOpen(position=1, x=1, y= 2) # first, tray num1 open automatically


    par_stuff_check.add_children([tray_1_full_seq,tray_1_empty_seq])
    tray_1_full_seq.add_children([tray1_full_check, tray_open_1])
    tray_1_empty_seq.add_children([tray1_empty_check, tray_2_check_par])

    tray_2_check_par.add_children([tray2_empty_seq, tray2_full_seq])
    tray2_empty_seq.add_children([tray2_empty_check, tray_open_1_2])
    tray2_full_seq.add_children([tray2_full_check, tray_open_2])

    tray_open1 = BoxOpen(position=1, x=1, y=1)


    root_sel = py_trees.composites.Selector('tray_unload2')
    ...
    root.add_children([check_critical_error(), tray_stuck_seq_new(), go_to_minor_error_tree(), item_return_check(), msgs_to_BB_parallel(), minor_error_alarm(), check_manual_drive(), check_manual_charge(), emergency_button_check(), battery_under_15(), time_delay_check_seq(), main_seq ])
    
    main_seq.add_children([bb_start, LED_charging_f, LED_charging_r, get_tray_info, tray_action_start_tts, task_available_alarm, user_password_check_seq(), tray_open1, tray_manual_par, go_to_next_state() ])
    # main_seq.add_children([bb_start, LED_charging_f, LED_charging_r, get_tray_info, tray_action_start_tts, task_available_alarm, user_password_check_seq(), par_stuff_check, tray_manual_par ])

    tray_manual_par.add_children([tray_action_seq_manual(), hri_finished_check])
