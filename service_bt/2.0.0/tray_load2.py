...
from kt_msgs.msg import BoxState, BoxStates, LEDState, Location, LEDEffect
...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_load2')
...
class unset_route_info(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        for i in range(py_trees.blackboard.Blackboard.get('poi_len')):
            py_trees.behaviours.UnsetBlackboardVariable('poi_success_check')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/waypoint_poi')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/map_name')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/zone')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/floor')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/is_ev')
            py_trees.behaviours.UnsetBlackboardVariable(str(i+1)+'/map_change')
...
def time_delay_check_seq():
    ...
    seq_no_res.add_children([no_response_check(time11='user_input_duration'), user_input_delay_alarm, tts_user_input_delayed, tray_action_seq_manual_nono, go_to_next_state()])
    return seq
...
def go_to_next_state():
    ...
    seq_full_stuff.add_children([par_full_stuff_check, for_moving(alarm_type='task_finished', tray_id=[1, 2])])
    ...
    seq_1_only_full.add_children([par_1_only_full, set_next_current_tray_id1, for_moving(alarm_type='task_fail', tray_id=[2])])
    par_1_only_full.add_children([tray1_full_check, tray2_empty_check])

    seq_2_only_full.add_children([par_2_only_full, set_next_current_tray_id2, for_moving(alarm_type='task_fail', tray_id=[1])])
    par_2_only_full.add_children([tray2_full_check, tray1_empty_check])

    seq_empty_stuff.add_children([par_empty_stuff_check, for_moving(alarm_type='task_fail', tray_id=[1, 2])])
    par_empty_stuff_check.add_children([empty_stuff_check, empty_stuff_check2])

    return seq

def for_moving(alarm_type, tray_id):
    ...
    seq.add_children([add_goal_index_num, unset_route, route_to_BB, par_route_ready_check, tts_delivering_resume, task_alarm, go_to_moving_tree])
...
def create_root():
    ...
    manual_trigger = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'manual_trigger',
                check=py_trees.common.ComparisonExpression(
                variable='hri_status',
                value='Tray_open',
    ...
    tray_open = BoxOpen(position=1, x=1, y= 1) # first, tray num1 open automatically

    root_sel = py_trees.composites.Selector('tray_load2')
    ...
    root.add_children([tray_stuck_seq_new(), check_critical_error(), go_to_minor_error_tree(), msgs_to_BB_parallel(), minor_error_alarm(), check_manual_drive(), check_manual_charge(), emergency_button_check(), battery_under_15(), time_delay_check_seq(), main_seq ])
    
    main_seq.add_children([BB_start, LED_charging_f, LED_charging_r, get_tray_info, tray_action_start_tts, task_available_alarm, user_password_check_seq(), tray_open, tray_manual_par, go_to_next_state() ])
