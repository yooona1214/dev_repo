...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_load1')
        py_trees.blackboard.Blackboard.set('now_goal_index_num', py_trees.blackboard.Blackboard.get('goal_index_num'))
...
class GetandSetOtherTrayBlackBoard(py_trees.behaviour.Behaviour):
    """
    BB init
    """
    ...
    def update(self):
        ...
        x = py_trees.blackboard.Blackboard.get('tray1')
        y = (x%2)+1
        
        self.a = 'Tray_'+str(y)+'_stuff_status'
        #py_trees.blackboard.Blackboard.set('abc', 'Tray_'+str(py_trees.blackboard.Blackboard.get('tray1'))+'_stuff_status')
        py_trees.blackboard.Blackboard.set(self.BB_variable, py_trees.blackboard.Blackboard.get(self.a))
...
class GetTrayInfo(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        current_tray_num = tray_list[0]
        py_trees.blackboard.Blackboard.set('tray1', current_tray_num) # tray1 / tray2
        
        #py_trees.blackboard.Blackboard.set('current_tray_pw', py_trees.blackboard.Blackboard.get('tray' +str(current_tray_num)+ '_pw') )
...
def msgs_to_BB_parallel():
    ...
    def on_msg_fn_pw_req(msg):
        ...
        py_trees.blackboard.Blackboard.set('user_pw_debug', msg.pw)
...
def time_delay_check_seq():
    ...
    seq_no_res.add_children([no_response_check(time11='user_input_duration'), user_input_delay_alarm, tts_user_input_delayed, go_to_next_state()])
    return seq
...
def user_password_check_seq():
    ...
    pw_correct_check = compareBBvariableforcorrect(BB_variable1 = 'current_tray_pw',
                                         BB_variable2 = 'user_pw')
    
    pw_incorrect_check = compareBBvariableforincorrect(BB_variable1 = 'current_tray_pw',
                                         BB_variable2 = 'user_pw')
    ...
    pw_incorrect_check2 = compareBBvariableforincorrect(BB_variable1 = 'current_tray_pw',
                                         BB_variable2 = 'user_pw')
    ...
    def request_generate_fn_tray_pw():
        ...
        req.type = 'auth'
        req.version = '1.0.0'
        req.file = 'user_tray_'+str(py_trees.blackboard.Blackboard.get('tray1'))+'_pw.json'
        return req
    ...
    def response_fn_tray_pw(response):
        ...
        py_trees.blackboard.Blackboard.set('current_tray_pw',pw)
...
def tray_action_seq():
    ...
    def request_generate_fn_tray_open():
        ...
        request.data.location.y = py_trees.blackboard.Blackboard.get('tray1')
        return request
    ...
    seq.add_children([tray_open, hri_finished_check, go_to_next_state()])
...
def go_to_next_state():
    ...
    par4 = py_trees.composites.Parallel(
        name="par4",
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    ...
    par5 = py_trees.composites.Parallel(
        name="par5",
        policy=py_trees.common.ParallelPolicy.SuccessOnAll(
            synchronise=False
        )
    ...
    seq_single = py_trees.composites.Sequence()
    seq_multi = py_trees.composites.Sequence()
    seq_none_stuff = py_trees.composites.Sequence()
    seq_full_stuff = py_trees.composites.Sequence()

    single_goal_count_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name='single_goal_count_check',
        check=py_trees.common.ComparisonExpression(
    ...
    multi_goal_count_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name='multi_goal_count_check',
        check=py_trees.common.ComparisonExpression(
        variable='current_goal_count',
        value=3,
        operator= operator.eq
    ...
    tray_current_set1 = GetandSetCurrentTrayBlackBoard(BB_variable='Tray_current_stuff_status')     
    tray_current_set2 = GetandSetCurrentTrayBlackBoard(BB_variable='Tray_current_stuff_status')    

    none_stuff_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'none_stuff_check',
        check=py_trees.common.ComparisonExpression(
    ...
    none_stuff_check2 = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'none_stuff_check2',
        check=py_trees.common.ComparisonExpression(
        variable="Tray_current_stuff_status",
        value=False,
        operator= operator.eq
    ...
    go_to_BB_clear_tree = EnqueueNextService(service_name='BB_clear', service_version='2.0.0')
    go_to_BB_clear_tree2 = EnqueueNextService(service_name='BB_clear', service_version='2.0.0')

    
    def request_generate_box_close():
        request = BoxCtrl.Request()
        ...
        request.data.location.y = py_trees.blackboard.Blackboard.get('tray1')
        return request
    def response_fn_box_close(response):
        print(response.success)

    tray_close = ServiceCall(
        service_name='hardware_manager/box/box_control',
        service_type= BoxCtrl,
        request_generate_fn=request_generate_box_close,
        response_fn= response_fn_box_close, name='tray_close'

    )

    task_fail_alarm_single_goal = PublishTopic(
                            name='task_fail_alarm_single_goal',
                            topic_name='ktmw_bt/task_status',
                            topic_type = TaskStatus, #TODO
                            msg_generate_fn=lambda: TaskStatus(
                                service_id=py_trees.blackboard.Blackboard.get('goal_index_num_current_service_id'),
    ...
    seq_multi_goal_none_stuff = py_trees.composites.Sequence()
    seq_multi_goal_full_stuff = py_trees.composites.Sequence()
    seq_goal_num_1 = py_trees.composites.Sequence()
    seq_goal_num_2 = py_trees.composites.Sequence()
    seq_other_tray_empty = py_trees.composites.Sequence()
    seq_other_tray_full = py_trees.composites.Sequence()

    goal_num_1_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'goal_num_1_check',
        check=py_trees.common.ComparisonExpression(
        variable="now_goal_index_num",
        value=1,
        operator= operator.eq
        )
    )

    goal_num_2_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'goal_num_2_check',
        check=py_trees.common.ComparisonExpression(
        variable="now_goal_index_num",
        value=2,
        operator= operator.eq
    ...
    other_tray_empty_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'other_tray_empty_check',
        check=py_trees.common.ComparisonExpression(
        variable="Tray_other_stuff_status",
        value=False,
        operator= operator.eq
        )
    )

    other_tray_full_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'other_tray_full_check',
        check=py_trees.common.ComparisonExpression(
        variable="Tray_other_stuff_status",
        value=True,
        operator= operator.eq
    ...
    other_tray_current_set = GetandSetOtherTrayBlackBoard(BB_variable='Tray_other_stuff_status')
    ...
    task_finished_alarm_task_not_done = PublishTopic(
                            name='task_finished_alarm_task_not_done',
                            topic_name='ktmw_bt/task_status',
                            topic_type = TaskStatus, #TODO
                            msg_generate_fn=lambda: TaskStatus(
                                service_id=py_trees.blackboard.Blackboard.get('goal_index_num_current_service_id'),
    ...
    par.add_children([seq_single, seq_multi])

    seq_single.add_children([single_goal_count_check, tray_current_set1, par2])
    par2.add_children([seq_none_stuff, seq_full_stuff])
    seq_none_stuff.add_children([none_stuff_check, task_fail_alarm_single_goal, go_to_BB_clear_tree])
    seq_full_stuff.add_children([full_stuff_check, for_moving(alarm_type='task_finished')])

    seq_multi.add_children([multi_goal_count_check, tray_current_set2, par3])
    par3.add_children([seq_multi_goal_full_stuff, seq_multi_goal_none_stuff])
    seq_multi_goal_full_stuff.add_children([full_stuff_check2, for_moving(alarm_type='task_finished')])

    seq_multi_goal_none_stuff.add_children([none_stuff_check2, par4])
    par4.add_children([seq_goal_num_1, seq_goal_num_2])
    seq_goal_num_1.add_children([goal_num_1_check, for_moving(alarm_type='task_fail')])
    seq_goal_num_2.add_children([goal_num_2_check, other_tray_current_set, par5])

    par5.add_children([seq_other_tray_empty, seq_other_tray_full])
    seq_other_tray_empty.add_children([other_tray_empty_check, task_fail_alarm, go_to_BB_clear_tree2])
    seq_other_tray_full.add_children([other_tray_full_check, for_moving(alarm_type='task_fail')])

    return seq

def for_moving(alarm_type):
    
    seq = py_trees.composites.Sequence()

    add_goal_index_num = Add_1('goal_index_num')

    unset_route = unset_route_info()


    #경로 to BB -> TBD
    def request_generate_fn_poi2bb():
        goal_index = py_trees.blackboard.Blackboard.get('goal_index_num')
        req = GetPoiRoute.Request()
        req.goal_label = py_trees.blackboard.Blackboard.get(str(goal_index)+'/current_goal_id')
        return req
    ...
    seq.add_children([add_goal_index_num, unset_route, route_to_BB, par_route_ready_check, tts_delivering_resume, task_finished_alarm, go_to_moving_tree])
...
def create_root():
    ...
    tray_action_start_tts= TTSPlay(tts_name='pickup_arrived', play=True, sequence=True)
    ...
    root_sel = py_trees.composites.Selector('tray_load1')
    ...
    main_seq.add_children([bb_start, LED_charging_f, LED_charging_r, get_tray_info,tray_action_start_tts, task_available_alarm, user_password_check_seq(), tray_action_seq(), go_to_next_state()])
    return root_sel
