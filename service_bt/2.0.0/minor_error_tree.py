...
from alarm_manager_msgs.msg import SetAlarm


# ** Required **
SERVICE_NAME = 'major-error-tree'
# ** Required **
...
def msgs_to_BB_parallel():
    ...
    par.add_children([emergency_button_info_to_BB, box_exception_to_BB, button_status_to_BB, battery_state_to_BB, charging_state_to_BB, user_input_msgs_to_BB, 
...
def emergency_button_check():
    ...
    led_start_on_minor_front = PublishTopic(
                name='led_start_on_minor_front',
                topic_name='led/set_led_effect_type',
                topic_type = LEDEffect,
                msg_generate_fn=lambda: LEDEffect(
                    cmd_id=0,
                    location=Location(position=1, x=1, y=1),
                    color=LEDState(cmd_id=1,r=255, g=165, b=0),
                    effect=2,
                    period=500,
                    on_ms=200,
                    repeat_count=0
    ...
    led_start_on_minor_rear = PublishTopic(
                name='led_start_on_minor_rear',
                topic_name='led/set_led_effect_type',
                topic_type = LEDEffect,
                msg_generate_fn=lambda: LEDEffect(
                    cmd_id=0,
...
def QR_recognize():
    ...
    manualdriving_off_check = py_trees.behaviours.WaitForBlackboardVariableValue(
        name= 'manual_drive_button_off_check',
        check=py_trees.common.ComparisonExpression(
            variable="button_manual_driving",
            value=False,
    ...
    seq.add_children([manual_drive_button_check, opened_tray_check_and_close(), Run_QR_Node, Driving_mode_set, qr_start_alarm, par_qr_status, qr_result_check, qr_success_alarm, tts_manualdriving_off, QR_Node_off, Driving_mode_off, manualdriving_off_check, go_to_BB_clear]) #manual_drive_off, manual_driving_off_alarm,
...
def create_root():
    ...
    error_available = PublishTopic(
                            ...
                            msg_generate_fn=lambda: TaskStatus(
                                ...
                                current_goal_id='',
                                tray_id=[0],             
                                task_status = 'error_qr',
                                action = 'error_qr',
                                action_target = 'error_qr',
                                action_result = True
                                )   
                            ) 
    ...
    root_par.add_children([msgs_to_BB_parallel(), check_manual_drive(), emergency_button_check(), time_delay_check_seq(), seq_next, tray_stuck_seq_new(), QR_recognize(), par_get_goods, check_installation_mode(), seq_main])
    ...
    seq_get_goods.add_children([admin_password_check_seq(), tray_action_seq_manual()])
    
    seq_next.add_children([hri_finished_check, opened_tray_check_and_close(), par_tray_all_closed])
    par_tray_all_closed.add_children([tray1_closed_check, tray2_closed_check])

    seq_main.add_children([BGM_off_cancel, error_available, task_available_alarm, led_start_on_Emergency_front, led_start_on_Emergency_rear, Idling_error])
