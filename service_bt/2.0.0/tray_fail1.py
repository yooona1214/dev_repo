...
SERVICE_NAME = 'tray_fail1'
SERVICE_VERSION = '0.1.0'


class CountCheck(py_trees.behaviour.Behaviour):
    """
    """
...
#    def initialise(self):
    ...
    def update(self):
        ...
        py_trees.blackboard.Blackboard.set('current_count', current_time-start_time)
...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_fail1')
...
class unset_route_info(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        for i in range(poi_len):
            py_trees.blackboard.Blackboard.set('poi_success_check',False)
...
class GetTrayInfo2(py_trees.behaviour.Behaviour):
    """
    BB init
    """
    ...
    def update(self):
        ...
        tray_list = py_trees.blackboard.Blackboard.get('from_fail2_tray_id') # array('B', [1, 2])

        py_trees.blackboard.Blackboard.set('tray1', tray_list[0]) # tray1 / tray2
...
def msgs_to_BB_parallel():
    ...
    par.add_children([error_set_to_BB, item_return_msgs_to_BB, button_status_to_BB, battery_state_to_BB, charging_state_to_BB, user_input_msgs_to_BB, 
                      admin_pw_from_ui_req_msgs_to_BB,  emergency_button_info_to_BB, tray_open_status_to_BB, remote_unlock_status_to_BB, box_exception_to_BB])
...
def tray_action_seq():
    ...
    par_stuff_check = py_trees.composites.Parallel(
        ...
        policy=py_trees.common.ParallelPolicy.SuccessOnSelected(
            children=[tray_object_sensing_empty_check],
            synchronise=False
        )
    )
...
def go_to_next_state():
    ...
    go_to_fail_again = EnqueueNextService(
    ...
    seq3.add_children([tray_object_sensing_full_check, task_fail_alarm, set_BB_fail, go_to_fail_again])
...
def create_root():
    ...
    root_sel = py_trees.composites.Selector('tray_fail1')
    ...
    seq_from_fail2 = py_trees.composites.Sequence()
    seq_from_other = py_trees.composites.Sequence()

    check_pre_tree1 = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_pre_tree1',
                check=py_trees.common.ComparisonExpression(
                variable='from_fail2',
                value=True,
                operator= operator.eq
    ...
    check_pre_tree2 = py_trees.behaviours.WaitForBlackboardVariableValue(
                name= 'check_pre_tree2',
                check=py_trees.common.ComparisonExpression(
                variable='from_fail2',
                value=False,
                operator= operator.eq
    ...
    main_seq.add_children([TTS_item_collect, bb_start, pw_check_set, LED_charging_f, LED_charging_r, par, task_available_alarm, admin_password_check_seq(), unset_admin_pw, tray_action_start_tts, tray_action_seq(), go_to_next_state()])
    par.add_children([seq_from_fail2, seq_from_other])
    seq_from_fail2.add_children([check_pre_tree1, get_tray_info2])
    seq_from_other.add_children([check_pre_tree2, get_tray_info])
    return root_sel
