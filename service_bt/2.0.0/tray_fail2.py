...
class BB_init(py_trees.behaviour.Behaviour):
    ...
    def setup(self, **kwargs):
        ...
        py_trees.blackboard.Blackboard.set('previous_tree', 'tray_fail2')
        py_trees.blackboard.Blackboard.set('from_fail2', True)
...
def time_delay_check_seq():
    ...
    seq_no_res.add_children([no_response_check(time11='user_input_duration'), tray_action_seq_manual_nono, user_input_delay_alarm, go_to_next_state()])
    return seq
...
def go_to_next_state():
    ...
    set_next_current_tray_id1 = SetBlackBoard(BB_variable='from_fail2_tray_id', BB_value=[1])
    set_next_current_tray_id2 = SetBlackBoard(BB_variable='from_fail2_tray_id', BB_value=[2])
    ...
    seq_full_stuff.add_children([par_full_stuff_check, for_cancel_2_again(alarm_type='task_fail', tray_id=[1, 2])])
    ...
    seq_1_only_full.add_children([par_1_only_full, set_next_current_tray_id1, for_cancel_1_again(alarm_type='task_fail', tray_id=[2])])
    par_1_only_full.add_children([tray1_full_check, tray2_empty_check])

    seq_2_only_full.add_children([par_2_only_full, set_next_current_tray_id2, for_cancel_1_again(alarm_type='task_fail', tray_id=[1])])
    par_2_only_full.add_children([tray2_full_check, tray1_empty_check])

    seq_empty_stuff.add_children([par_empty_stuff_check, for_idle(alarm_type='task_finished', tray_id=[1,2])])
    par_empty_stuff_check.add_children([empty_stuff_check, empty_stuff_check2])

    return seq

def for_idle(alarm_type, tray_id):
...
def for_cancel_1_again(alarm_type, tray_id):
    ...
    go_to_cancel_1_tree = EnqueueNextService(
        service_name='tray_fail1', service_version=''
    )

    seq.add_children([task_alarm, go_to_cancel_1_tree])
    return seq

def for_cancel_2_again(alarm_type, tray_id):
    ...
    go_to_cancel_2_tree = EnqueueNextService(
        service_name='tray_fail2', service_version=''
    )

    seq.add_children([task_alarm, go_to_cancel_2_tree])
    return seq
...
def msgs_to_BB_parallel():
    ...
    par.add_children([error_set_to_BB, item_return_msgs_to_BB, button_status_to_BB, battery_state_to_BB, charging_state_to_BB, user_input_msgs_to_BB, 
                      admin_pw_req_msgs_to_BB,  emergency_button_info_to_BB, tray_open_status_to_BB, remote_unlock_status_to_BB, box_exception_to_BB])
...
def admin_password_check_seq():
    ...
    seq6.add_children([pw_correct_check, auth_checkced_alarm, led_start_on_pw_done_f, led_start_on_pw_done_r])
...
def create_root():
    ...
    TTS_item_collect = TTSPlay(tts_name='item_collect', play=True, sequence=True)

    root_sel = py_trees.composites.Selector('tray_fail2')
    ...
    root.add_children([ check_critical_error(), go_to_minor_error_tree(), check_manual_drive(), minor_error_alarm(), check_manual_charge(), emergency_button_check(), battery_under_15(), msgs_to_BB_parallel(), time_delay_check_seq(), tray_stuck_seq_new(), main_seq ])
    
    main_seq.add_children([TTS_item_collect, bb_start, pw_check_set, LED_charging_f, LED_charging_r, get_tray_info, task_available_alarm, admin_password_check_seq(), unset_admin_pw, tray_action_start_tts, tray_open, tray_manual_par, go_to_next_state()])
